...
export function userFacingErrorMessage(error: unknown, fallback = defaultErrorMessage) {
  return userFacingMessageFromText(error instanceof Error ? error.message : String(error), fallback)
}

export function userFacingMessageFromText(message: string | undefined, fallback = defaultErrorMessage) {
...
export function isReadOnlyWalletMessage(message: string | undefined) {
  return /local wallet is read-only|local dev wallet cannot submit/i.test(String(message ?? ''))
}

export function isRevealTooEarlyMessage(message: string | undefined) {
  return /reveal too early/i.test(String(message ?? ''))
}

export function isTechnicalErrorMessage(message: string | undefined) {
...
export function isWalletLockedMessage(message: string | undefined) {
...
export function isRejectedWalletMessage(message: string | undefined) {
  return /reject|denied|cancel/i.test(String(message ?? ''))
}

function isNameDataUnavailableMessage(message: string) {
